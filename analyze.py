...
def main():
    ...
    if len(sys.argv) != 2:
        print(f"Usage: python {sys.argv[0]} <input_code_file>")
        sys.exit(1)
    ...
    final_output += (
        ...
        f"Time per input token:       {format_time(time_per_input_token)}\n"
        f"Time per generated token:   {format_time(time_per_generated_token)}\n"
        "========================================\n"
    )
    os.makedirs(OUTPUT_DIRECTORY, exist_ok=True)
